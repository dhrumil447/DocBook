  ...
  const fetchDoctorDetails = async (doctorId) => {
    try {
      ...
      if (response.ok) {
        ...
        // Backend uses consultation_fee field
        setDoctorFee(data.consultation_fee || data.fees || 500);
        console.log("Doctor fee set to:", data.consultation_fee || data.fees);
      } else {
        toast.error("Failed to fetch doctor details");
        setDoctorFee(500); // Default fee
      }
    } catch (error) {
  ...
  const storeAppointment = async (paymentMethod, razorpayId = "") => {
    ...
    try {
      ...
      if (appointmentResponse.ok) {
        ...
        if (!patientId || !doctor.id || !doctorFee || !paymentMethod) {
        ...
        const paymentData = {
          ...
          doctor_id: doctor.id,
          appointment_id: appointment.id, // Store appointment ID
          amount: doctorFee,
          payment_method: paymentMethod,
          razorpay_payment_id: razorpayId,
          payment_status: razorpayId ? "Completed" : "Pending",
        };
        ...
        if (paymentResponse.ok) {
          toast.success(
            "Appointment booked and payment recorded successfully!",
          );
          handleClose();
        } else {
          ...
          <>
            {doctor && (
              ...
              >
                ...
                <div>
                  <h5
                    style={{ margin: 0, color: "#2c3e50", fontWeight: "bold" }}
                  >
                    Dr. {doctor.username}
                  </h5>
                    ...
                    >
                      {slots[date].slots.map((slot, i) => (
                        <Button
                          key={i}
                          variant={
                            selectedSlot === slot && selectedDate === date
                              ? "primary"
                              : "outline-primary"
                          }
                          style={{
          ...
          >
            ...
            >
              <div
                ...
                style={{
                  background:
                    "linear-gradient(135deg, #19875411 0%, #19875422 100%)",
