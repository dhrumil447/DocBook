...
const UserPrescription = () => {
  ...
  const fetchPrescriptions = async (id) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/medicalReports?patientId=${id}`,
      );

      const patientRes = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/patients/${id}`,
      ); // Fetch patient details

      const prescriptionsWithDetails = await Promise.all(
        response.data.map(async (prescription) => {
          const doctorRes = await axios.get(
            `${import.meta.env.VITE_BASE_URL}/doctors/${prescription.doctor_id}`,
          );
          const appointmentRes = await axios.get(
            `${import.meta.env.VITE_BASE_URL}/appointments/${prescription.appointment_id}`,
          );

          return {
            ...prescription,
            doctorName: doctorRes.data.username,
            appointmentDate: appointmentRes.data.appointment_date,
            appointmentTime: appointmentRes.data.appointment_time,
  ...
  const generatePDF = (prescription) => {
    ...
    doc.text(
      `Appointment Date: ${moment(prescription.appointmentDate).format("DD-MM-YYYY")}`,
      14,
      85,
    );
    ...
    if (prescription.nextAppointmentDate) {
      doc.text(
        `Next Appointment Date: ${moment(prescription.nextAppointmentDate).format("DD-MM-YYYY")}`,
        14,
        115,
      );
    }
    ...
    doc.save(`Prescription_${prescription.appointment_id}.pdf`);
  };
      ...
      <div className="container" style={{ maxWidth: "1200px" }}>
        ...
        {filteredPrescriptions.length === 0 ? (
          <Card
            ...
            style={{ borderRadius: "15px", padding: "60px 20px" }}
          >
          ...
          filteredPrescriptions.map((prescription, index) => (
            ...
            >
              ...
              >
                ...
                <small>
                  {moment(prescription.appointmentDate).format("DD MMMM YYYY")}{" "}
                  at {prescription.appointmentTime}
                </small>
              </div>
              ...
              <Card.Body className="p-4">
                ...
                <h6 className="fw-bold mt-4 mb-3" style={{ color: "#2c3e50" }}>
                ...
                >
                  <thead style={{ backgroundColor: "#f8f9fa" }}>
                    <tr>
                      <th style={{ fontWeight: "600", color: "#2c3e50" }}>
                        Medicine Name
                      </th>
                      <th style={{ fontWeight: "600", color: "#2c3e50" }}>
                        Dosage
