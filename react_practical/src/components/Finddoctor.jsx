...
const Finddoctor = () => {
  ...
  const filterDoctors = () => {
    ...
    if (searchName) {
      updatedList = updatedList.filter((doctor) =>
        doctor.username.toLowerCase().includes(searchName.toLowerCase()),
      );
    }
  ...
  const handleBookAppointment = async (doctor) => {
    ...
    // Fetch slots from backend for the specific doctor
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/slots?doctor_id=${doctor.id}`,
      );
      const doctorSlots = response.data || [];

      if (doctorSlots.length === 0) {
        toast.info("No available slots for this doctor at the moment");
      }

      // Transform backend slots data into the format expected by SlotSelectionModal
      ...
      doctorSlots.forEach((slot) => {
        ...
        formattedSlots[formattedDate].slots.push(slot.slot_time);
      });
            ...
            <Row className="align-items-center">
              <Col md={8}>
                ...
                >
                  ...
                  >
                    Quickly connect with expert healthcare professionals near
                    you
                  </p>
                </motion.div>
              </Col>
                ...
                >
                  <Card.Body className="p-4">
                    <h5 className="fw-bold d-flex align-items-center mb-4">
                      <FaFilter className="me-2 text-primary" /> Filter by
                      Specialization
                    </h5>
                    <ListGroup variant="flush">
                      <ListGroup.Item
                        ...
                        style={{
                          ...
                          backgroundColor: !selectedSpecialization
                            ? "#0d6efd"
                            : "transparent",
                      ...
                      {specializations.map((spec, index) => (
                        <ListGroup.Item
                          ...
                          style={{
                            cursor: "pointer",
                            fontWeight:
                              selectedSpecialization === spec ? "600" : "400",
                            backgroundColor:
