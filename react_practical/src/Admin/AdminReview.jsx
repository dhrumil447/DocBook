...
const AdminReview = () => {
  ...
  const doctorAverages = useMemo(() => {
    ...
    reviews.forEach(review => {
      const docId = review.doctor_id || review.doctorId;
      if (!averages[docId]) averages[docId] = { total: 0, count: 0 };
      averages[docId].total += review.rating;
      averages[docId].count += 1;
    });
    Object.keys(averages).forEach(docId => {
      averages[docId] = averages[docId].total / averages[docId].count;
    });
    return averages;
  }, [reviews]);
                    ...
                    <td style={{ padding: "15px", verticalAlign: "middle" }}>
                      ...
                      >
                        ...
                        <span style={{ fontWeight: "600", color: "#2c3e50" }}>
                          {getDoctorName(review.doctor_id || review.doctorId)}
                        </span>
                        <br />
                        <small style={{ color: "#6c757d", fontSize: "12px" }}>
                          Avg Rating: {doctorAverages[review.doctor_id || review.doctorId]?.toFixed(1) || 0} ⭐
                        </small>
                      </div>
                    </td>
                    <td style={{ padding: "15px", verticalAlign: "middle" }}>
                      ...
                      >
                        ...
                        <span style={{ color: "#495057" }}>
                          {getPatientName(
                            review.patient_id || review.patientId,
                          )}
                        </span>
                      </div>
                    </td>
                      ...
                      {review.status === "Pending" && (
                        ...
                        >
                          ...
                          <Button
                            size="sm"
                            onClick={() =>
                              handleUpdateStatus(review.id, "Canceled")
                            }
                            style={{
                              background:
                                "linear-gradient(135deg, #dc3545, #c82333)",
