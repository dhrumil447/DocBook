  ...
  const fetchPayments = async () => {
    try {
      ...
      const onlineTotal = payments
        .filter((p) => (p.payment_method || p.paymentMethod) === "Online")
        .reduce((sum, p) => sum + parseFloat(p.amount || 0), 0);
      const codTotal = payments
        .filter((p) => (p.payment_method || p.paymentMethod) === "COD")
        .reduce((sum, p) => sum + parseFloat(p.amount || 0), 0);
  ...
  const fetchTopDoctor = async () => {
    try {
      ...
      const doctorCount = appointments.reduce((acc, app) => {
        const doctorId = app.doctor_id || app.doctorId;
        if (doctorId) {
      ...
      const doctorIds = Object.keys(doctorCount);
      if (doctorIds.length === 0) {
        return;
      }

      const topDoctorId = doctorIds.reduce((a, b) =>
        doctorCount[a] > doctorCount[b] ? a : b,
      );

      if (topDoctorId) {
        const doctorRes = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/doctors/${topDoctorId}`,
        );
  ...
  const fetchHighestRatedDoctor = async () => {
    try {
      ...
      const doctorRatings = reviews.reduce((acc, review) => {
        const doctorId = review.doctor_id || review.doctorId;
        if (doctorId) {
          acc[doctorId] = acc[doctorId] || { total: 0, count: 0 };
          acc[doctorId].total += review.rating;
          acc[doctorId].count += 1;
        }
        return acc;
      }, {});

      const doctorIds = Object.keys(doctorRatings);
      if (doctorIds.length === 0) {
        return;
      }

      const highestRatedId = doctorIds.reduce((a, b) =>
        doctorRatings[a].total / doctorRatings[a].count >
        doctorRatings[b].total / doctorRatings[b].count
          ? a
          : b,
      );
      ...
      if (highestRatedId) {
        const doctorRes = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/doctors/${highestRatedId}`,
        );
        setHighestRatedDoctor({
          ...doctorRes.data,
          rating: (
            doctorRatings[highestRatedId].total /
            doctorRatings[highestRatedId].count
          ).toFixed(1),
        });
      }
    } catch (err) {
        ...
        <Col md={4} className="mb-4">
          ...
          >
            ...
            <Card.Body style={{ textAlign: "center", padding: "20px" }}>
              ...
              >
                {highestRatedDoctor
                  ? `Dr. ${highestRatedDoctor.username} (${highestRatedDoctor.rating} ⭐)`
                  : "No data"}
              </Card.Text>
            </Card.Body>
          </Card>
